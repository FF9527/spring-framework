...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// SavepointAllowed：是否支持事务嵌套，是否支持保存点
		//connectionHolder：连接管理，ThreadLocal实现，从当前线程中获取数据库连接
		...
		if (isExistingTransaction(transaction)) {
			...
			// 已存在事务，根据事务传播级别，进行相应处理并返回相应的TransactionStatus
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
			//传播级别为required_new且当前线程已存在事务时，暂停事务，创建新事务执行完后，还原事务
			if (debugEnabled) {
			...
			//清除线程中的事务信息（ThreadLocal.remove）
			//记录清除的事务信息到TransactionStatus的suspendedResources属性中，
			//新建的事务执行完后，拿出suspendedResources继续执行事务
